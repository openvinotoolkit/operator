...
style:
	docker run --rm -v $$(pwd):/app -w /app -e https_proxy=$(https_proxy) golangci/golangci-lint:v1.64.8 golangci-lint run -E stylecheck --disable-all -v --timeout 4m0s

lint:
	docker run --rm -v $$(pwd):/app -w /app -e https_proxy=$(https_proxy) golangci/golangci-lint:v1.64.8 golangci-lint run --skip-dirs ../go/pkg/mod -v --timeout 4m0s
